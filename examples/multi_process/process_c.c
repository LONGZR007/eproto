...
void c_receive_callback(uint8_t source_address, uint16_t packet_id, uint8_t* data, uint16_t length) {
...
void c_bus_send(uint8_t* data, uint16_t length) {
    printf("Process C sending: ");
    for (uint16_t i = 0; i < length; i++) {
        printf("%02X ", data[i]);
    }
    printf("\n");
...
void* receive_thread(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            printf("Process C received %d bytes: ", received);
            ...
            for (int i = 0; i < received; i++) {
                eproto_receive_byte(&g_eproto, 0x04, rx_buffer[i]);
            }
        }
        usleep(10000);
    }
...
int main(void) {
    ...
    eproto_bus_t c_bus = {.send = c_bus_send, .receive = NULL};

    uint8_t c_rx_buffer[256];
    error = eproto_add_bus(&g_eproto, 0x04, &c_bus, c_rx_buffer, sizeof(c_rx_buffer), "c_bus", mock_wakeup,
    ...
    error = eproto_add_destination_device(&g_eproto, 0x04, 0x01);
    if (error != EPROTO_OK) {
    ...
    error = eproto_add_destination_device(&g_eproto, 0x04, 0x03);
    if (error != EPROTO_OK) {
    ...
    if (ipc_open_fifo(&g_ipc_rx_channel, FIFO_PATH_B_TO_C) < 0) {
        printf("Failed to open RX FIFO\n");
        return 1;
    }

    if (ipc_open_fifo(&g_ipc_tx_channel, FIFO_PATH_C_TO_B) < 0) {
                    ...
                    if (rest) {
                        ...
                        if (rest) {
                            ...
                            if (data_len > 0) {
                                ...
                                error_cmd = eproto_send(&g_eproto, device_addr, data, data_len, c_send_callback, NULL,
                                                        need_reply ? 0 : 1);
