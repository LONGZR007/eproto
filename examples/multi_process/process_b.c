...
void b_bus_send_a(uint8_t* data, uint16_t length) {
    printf("Process B sending to A: ");
    for (uint16_t i = 0; i < length; i++) {
        printf("%02X ", data[i]);
    }
    printf("\n");
...
void b_bus_send_c(uint8_t* data, uint16_t length) {
    printf("Process B sending to C: ");
    for (uint16_t i = 0; i < length; i++) {
        printf("%02X ", data[i]);
    }
    printf("\n");
...
void* receive_thread_a(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            ...
            for (int i = 0; i < received; i++) {
                eproto_receive_byte(&g_eproto, 0x02, rx_buffer[i]);
            }
        }
        usleep(10000);
    }
...
void* receive_thread_c(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            ...
            for (int i = 0; i < received; i++) {
                eproto_receive_byte(&g_eproto, 0x03, rx_buffer[i]);
            }
        }
        usleep(10000);
    }
...
int main(void) {
    ...
    eproto_bus_t b_bus_a = {.send = b_bus_send_a, .receive = NULL};

    uint8_t b_rx_buffer_a[256];
    error = eproto_add_bus(&g_eproto, 0x02, &b_bus_a, b_rx_buffer_a, sizeof(b_rx_buffer_a), "b_bus_a", mock_wakeup,
                           mock_status_callback, b_receive_callback);
    if (error != EPROTO_OK) {
        printf("Failed to add bus A\n");
        return 1;
    }
    printf("Bus A added successfully\n");
    ...
    eproto_bus_t b_bus_c = {.send = b_bus_send_c, .receive = NULL};

    uint8_t b_rx_buffer_c[256];
    error = eproto_add_bus(&g_eproto, 0x03, &b_bus_c, b_rx_buffer_c, sizeof(b_rx_buffer_c), "b_bus_c", mock_wakeup,
                           mock_status_callback, b_receive_callback);
    if (error != EPROTO_OK) {
        printf("Failed to add bus C\n");
        return 1;
    }
    printf("Bus C added successfully\n");
                    ...
                    if (rest) {
                        ...
                        if (rest) {
                            ...
                            if (data_len > 0) {
                                ...
                                error_cmd = eproto_send(&g_eproto, device_addr, data, data_len, b_send_callback, NULL,
                                                        need_reply ? 0 : 1);
                                if (error_cmd != EPROTO_OK) {
