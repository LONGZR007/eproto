...
void mock_receive_callback(uint8_t source_address, uint16_t packet_id, uint8_t* data, uint16_t length) {
    (void)source_address;
    (void)packet_id;
    (void)data;
    (void)length;
...
void* device1_thread(void* arg) {
    ...
    error = eproto_add_bus(&data->eproto_inst, 0x01, &device1_bus, data->rx_buffer, sizeof(data->rx_buffer),
                           "device1_bus", mock_wakeup, mock_status_callback, mock_receive_callback);
    if (error != EPROTO_OK) {
...
void* device2_thread(void* arg) {
    ...
    error = eproto_add_bus(&data->eproto_inst, 0x02, &device2_bus, data->rx_buffer, sizeof(data->rx_buffer),
                           "device2_bus", mock_wakeup, mock_status_callback, mock_receive_callback);
    if (error != EPROTO_OK) {
        printf("%s: Failed to add bus\n", data->device_name);
        fflush(stdout);
        pthread_exit(NULL);
