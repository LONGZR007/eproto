...
void* receive_thread_b(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            ...
            for (int i = 0; i < received; i++) {
                eproto_receive_byte(&g_eproto, 0x06,
                                    rx_buffer[i]);  // 总线6接收来自B的数据
            }
        }
        usleep(10000);
    }
...
void* receive_thread_d(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            ...
            for (int i = 0; i < received; i++) {
                eproto_receive_byte(&g_eproto, 0x07,
                                    rx_buffer[i]);  // 总线7接收来自D的数据
            }
        }
        usleep(10000);
    }
...
int main(void) {
    ...
    // 总线6（连接到A1、B4、E11）
    eproto_bus_t c_bus6 = {.send = c_bus6_send, .receive = NULL};
    ...
    // 总线7（连接到D9）
    eproto_bus_t c_bus7 = {.send = c_bus7_send, .receive = NULL};
