...
void a_bus1_send(uint8_t* data, uint16_t length) {
    printf("Process A bus1 sending: ");
    for (uint16_t i = 0; i < length; i++) {
        printf("%02X ", data[i]);
    }
    printf("\n");
...
void a_bus2_send(uint8_t* data, uint16_t length) {
    printf("Process A bus2 sending: ");
    for (uint16_t i = 0; i < length; i++) {
        printf("%02X ", data[i]);
    }
    printf("\n");
...
void* receive_thread_b(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            printf("Process A received from B %d bytes: ", received);
            for (int i = 0; i < received; i++) {
                printf("%02X ", rx_buffer[i]);
            }
            printf("\n");

            eproto_receive_data(&g_eproto, 0x01, rx_buffer, received);  // 总线1接收来自B的数据
        }
        usleep(10000);
    }
    return NULL;
...
void* receive_thread_d(void* arg) {
    ...
    while (1) {
        ...
        if (received > 0) {
            printf("Process A received from D %d bytes: ", received);
            for (int i = 0; i < received; i++) {
                printf("%02X ", rx_buffer[i]);
            }
            printf("\n");

            eproto_receive_data(&g_eproto, 0x02, rx_buffer, received);  // 总线2接收来自D的数据
        }
        usleep(10000);
    }
    return NULL;
...
int main(void) {
    ...
    // 总线1（连接到B3、C6、E11）
    eproto_bus_t a_bus1 = {.send = a_bus1_send, .receive = NULL};

    uint8_t a_rx_buffer1[256];
    error = eproto_add_bus(&g_eproto, 0x01, &a_bus1, a_rx_buffer1, sizeof(a_rx_buffer1), "a_bus1", mock_wakeup,
                           mock_status_callback);
    if (error != EPROTO_OK) {
        printf("Failed to add bus 1\n");
        return 1;
    }
    printf("Bus 1 added successfully\n");

    // 总线2（连接到D8）
    eproto_bus_t a_bus2 = {.send = a_bus2_send, .receive = NULL};

    uint8_t a_rx_buffer2[256];
    error = eproto_add_bus(&g_eproto, 0x02, &a_bus2, a_rx_buffer2, sizeof(a_rx_buffer2), "a_bus2", mock_wakeup,
                           mock_status_callback);
    if (error != EPROTO_OK) {
        printf("Failed to add bus 2\n");
        return 1;
    }
    printf("Bus 2 added successfully\n");

    // 添加目标设备
    // 总线1挂载到B3、C6、E11
    error = eproto_add_destination_device(&g_eproto, 0x01, 0x03);  // B
    if (error != EPROTO_OK) {
    ...
    error = eproto_add_destination_device(&g_eproto, 0x01, 0x06);  // C
    if (error != EPROTO_OK) {
    ...
    error = eproto_add_destination_device(&g_eproto, 0x01, 0x0B);  // E
    if (error != EPROTO_OK) {
    ...
    // 总线2挂载到D8
    error = eproto_add_destination_device(&g_eproto, 0x02, 0x08);  // D
    if (error != EPROTO_OK) {
    ...
    if (ipc_open_fifo(&g_ipc_tx_channel_b, FIFO_PATH_A_TO_B) < 0) {
        printf("Failed to open TX FIFO to B\n");
        return 1;
    }

    if (ipc_open_fifo(&g_ipc_rx_channel_b, FIFO_PATH_B_TO_A) < 0) {
        printf("Failed to open RX FIFO from B\n");
        return 1;
    }

    if (ipc_open_fifo(&g_ipc_tx_channel_d, FIFO_PATH_A_TO_D) < 0) {
        printf("Failed to open TX FIFO to D\n");
        return 1;
    }

    if (ipc_open_fifo(&g_ipc_rx_channel_d, FIFO_PATH_D_TO_A) < 0) {
